...
enum ReaderProbe {
    ...
    ///
    /// `TraceLog`'s, because the shippable trace needs the same number and two copies of
    /// one `task_info` call would drift.
    private static func residentMB() -> Int { TraceLog.residentMB() }
}
#endif
